    ...
    OpenIDAuthorizationCodeInterface
{
    ...
    public function getAccessToken($access_token){
        ...
        if ($token = @array_change_key_case(oci_fetch_assoc($stmt),CASE_LOWER)) {
			...
			//$date=("j-M-Y","15-Mar-2013");
			
			$date=date_create_from_format("d/m/y H:i:s.u",$token['expires']);
			$date=date_format($date,"Y/m/d H:i:s.u");
            $token['expires'] = strtotime($date);
			//echo $token['expires'];
    ...
    public function setAccessToken($access_token, $client_id, $user_id, $expires, $scope = null){
        ...
        if ($this->getAccessToken($access_token)) {
        	...
        	$strSql = sprintf("INSERT INTO %s (access_token, client_id, expires, user_id, scope) VALUES (:access_token, :client_id, TO_TIMESTAMP(:expires, 'YYYY-MM-DD HH24:MI:SS.FF'), :user_id, :scope)",$this->config['access_token_table']);
			$stmt = oci_parse($this->db,$strSql);
        }
    ...
    public function getAuthorizationCode($code){
        ...
        if ($code = @array_change_key_case(oci_fetch_assoc($stmt),CASE_LOWER)) {
            // convert date string back to timestamp
			$date=date_create_from_format("d/m/y H:i:s.u",$code['expires']);
			$date=date_format($date,"Y/m/d H:i:s.u");
            $code['expires'] = strtotime($date);
        }
        return $code;
    }
